module.exports = function () {
  ...
  this.When(/^the user has selected products and added them to the cart$/, async function () {
    ...
    for (let product of products) {
      ...
      let buyProduct = await (await product.findElement(By.css('button[title="Öka antal"]')));
  ...
  this.When(/^the user clicks on the cart icon$/, async function () {
    ...
    let goToCart = (await driver.findElement(By.css('a[href="https://www.willys.se/varukorg"]')));
  ...
  this.Then(/^the user should see the selected products$/, async function () {
    let checkCartView = (await driver.findElement(By.css('p[class="Text_text__1DvUt Text_body__FrvAc Text_black__yB42g Text_small__3m75u"')).getText());
  ...
  this.Then(/^the user will have the option to buy them$/, async function () {
    ...
    let goToCheckOut = (await driver.findElement(By.css('button[class="Buttonstyles__StyledButton-sc-1g4oxwr-0 gqUoiG"]')));
    await goToCheckOut.click();
    await driver.sleep(2000);

    let CheckOut = (await driver.findElement(By.css('span[class="checkout-button-text"]')));
