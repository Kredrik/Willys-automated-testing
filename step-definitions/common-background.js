...
module.exports = function () {
  ...
  this.Given(/^that we accepted the standard cookie policy$/, async function () {
    ...
    await driver.wait(until.elementsLocated
      (by.css('#onetrust-accept-btn-handler')), 10000);
    // once it exists grab the button
    let cookieAcceptButton = await driver.findElement
  ...
  this.Given(/^that we have been through the initial where to deliver popup$/, async function () {
    ...
    let plusButton = await driver.findElement
      (By.css('.ax-product-quantity-plus'));
    ...
    await driver.wait(until.elementsLocated
      (by.css('.ax-delivery-widget-overlay')), 10000);
    // Then grab it and click it
    let deliveryOverlay = await driver.findElement
      (By.css('.ax-delivery-widget-overlay'));
    await deliveryOverlay.click();
    // Grab the minus-button of the first product and click it 
    let minusButton = await driver.findElement
      (By.css('.ax-product-quantity-minus'));
